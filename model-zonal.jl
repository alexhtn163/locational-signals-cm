...
sigma = 0.5   # cross-zonal participation factor — sweep this across runs
...
largest_unit_z = Dict(z => maximum(
    (cap[g] for g in vcat(I, J) if gen_to_zone[g] == z && alpha[g] > 0.0);
    init=0.0)
    for z in Z)
...
d_c_z = Dict(z => maximum(
    ...
    for t in T) + largest_unit_z[z]
    for z in Z)

# System-wide: same logic
...
mec_import = Dict(z => sum(get(MEC, (z2, z), 0.0) for z2 in Z if z2 != z) for z in Z)
...
K_stat_cm  = lambda_c_ub + K_mu_mec   # -λ_c[z] + μ_cm_bud + μ_mec[z]
K_stat_ls  = VOLL
...
# Zonal clearing: internal + imports == requirement
@constraint(min, cm_clearing[z in Z],
    sum(c_cm_I[i, z] for i in I) +
    sum(c_cm_J[j, z] for j in J) == d_c_z[z])

# Generator CM budget: total offers across zones ≤ capacity
@constraint(min, cm_budget_I[i in I],
    sum(c_cm_I[i, z] for z in Z) <= alpha[i] * c[i])
@constraint(min, cm_budget_J[j in J],
    sum(c_cm_J[j, z] for z in Z) <= alpha[j] * cap[j])

# MEC: total cross-zonal imports into zone z ≤ sum of MEC from neighbours
...
@variable(min, 0 <= lambda_c[z in Z] <= lambda_c_ub)         # zonal capacity price
@variable(min, 0 <= mu_g_up_I[t in T, i in I] <= K_mu_g_I)
@variable(min, 0 <= mu_g_up_J[t in T, j in J] <= K_mu_g_J)
@variable(min, 0 <= mu_cm_bud_I[i in I] <= K_mu_cm_bud_I)    # dual on CM budget
@variable(min, 0 <= mu_cm_bud_J[j in J] <= K_mu_cm_bud_J)
@variable(min, 0 <= mu_cap_up[i in I] <= cap_up_ub)
@variable(min, 0 <= mu_mec[z in Z] <= K_mu_mec)              # dual on MEC constraint

# --- Stationarity conditions ---

# ∂L/∂q[t,i]: -(λ_e[t] - vc[i]) + μ_g_up_I[t,i] ≥ 0
@constraint(min, [t in T, i in I],
    -(lambda_e[t] - vc[i]) + mu_g_up_I[t, i] >= 0)

# ∂L/∂q[t,j]: -(λ_e[t] - vc[j]) + μ_g_up_J[t,j] ≥ 0
@constraint(min, [t in T, j in J],
    -(lambda_e[t] - vc[j]) + mu_g_up_J[t, j] >= 0)

# ∂L/∂c[i]: fc - sigma_t(μ_g_up_I[t,i] · a[t,i]) + μ_cap_up[i] - μ_cm_bud_I[i] · α[i] ≥ 0
@constraint(min, stat_c[i in I],
    fc[tech[i]]
    - sum(mu_g_up_I[t, i] * a[(t, i)] for t in T)
    + mu_cap_up[i]
    - mu_cm_bud_I[i] * alpha[i] >= 0)

# ∂L/∂c_cm_I[i,z]: -λ_c[z] + μ_cm_bud_I[i] + μ_mec[z]·𝟙(i∉zone z) ≥ 0
@constraint(min, stat_cm_I[i in I, z in Z],
    -lambda_c[z] + mu_cm_bud_I[i] +
    (gen_to_zone[i] != z ? mu_mec[z] : 0.0) >= 0)

# ∂L/∂c_cm_J[j,z]: -λ_c[z] + μ_cm_bud_J[j] + μ_mec[z]·𝟙(j∉zone z) ≥ 0
@constraint(min, stat_cm_J[j in J, z in Z],
    -lambda_c[z] + mu_cm_bud_J[j] +
...
@variable(min, r_cm_I[i in I, z in Z], Bin)         # for c_cm_I[i,z] ⊥ stat
@variable(min, r_cm_J[j in J, z in Z], Bin)          # for c_cm_J[j,z] ⊥ stat
@variable(min, r_cm_bud_I[i in I], Bin)              # for (budget slack) ⊥ μ_cm_bud_I
@variable(min, r_cm_bud_J[j in J], Bin)
@variable(min, r_mec[z in Z], Bin)                   # for (mec slack) ⊥ μ_mec
...
# --- Generator I: 0 ≤ c ⊥ (fc - sigmaμ·a + μ_cap - μ_cm_bud·α) ≥ 0 ---
@constraint(min, [i in I],
    c[i] <= K_c * (1 - r_c[i]))
@constraint(min, [i in I],
    fc[tech[i]] - sum(mu_g_up_I[t, i] * a[(t, i)] for t in T)
    + mu_cap_up[i] - mu_cm_bud_I[i] * alpha[i] <= K_stat_c * r_c[i])
...
# --- Generator I: 0 ≤ (cm budget slack) ⊥ μ_cm_bud_I ≥ 0 ---
@constraint(min, [i in I],
    alpha[i] * c[i] - sum(c_cm_I[i, z] for z in Z) <= K_slack_cm_bud_I * (1 - r_cm_bud_I[i]))
@constraint(min, [i in I],
    mu_cm_bud_I[i] <= K_mu_cm_bud_I * r_cm_bud_I[i])
...
# --- Generator J: 0 ≤ (cm budget slack) ⊥ μ_cm_bud_J ≥ 0 ---
@constraint(min, [j in J],
    alpha[j] * cap[j] - sum(c_cm_J[j, z] for z in Z) <= K_slack_cm_bud_J * (1 - r_cm_bud_J[j]))
@constraint(min, [j in J],
    mu_cm_bud_J[j] <= K_mu_cm_bud_J * r_cm_bud_J[j])
...
all_invest    = DataFrame(run_id=Int[], generator=String[], bus=String[], tech=String[], zone=Int[], cap_max=Float64[], c=Float64[], vc=Float64[], fc=Float64[], mu_cap_up=Float64[], mu_cm_bud=Float64[], energy_rent=Float64[], cm_rent=Float64[], total_rent=Float64[], fixed_cost=Float64[], profit=Float64[])
all_legacy    = DataFrame(run_id=Int[], generator=String[], bus=String[], zone=Int[], cap=Float64[], vc=Float64[], mu_cm_bud=Float64[], energy_rent=Float64[], cm_rent=Float64[], total_gen=Float64[])
...
for (k, w) in enumerate(all_weights)
    ...
    # ================================================================
    # Redispatch model (unchanged from original)
    # ================================================================
...
run_name = "zonal_cm_sigma$(Int(sigma*100))_nl34"
