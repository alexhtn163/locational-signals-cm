...
#weight_selection = :all
weight_selection = [29, 46]        
...
for sigma in sigmas
    ...
    av_file = "results/final_$(scenario)_sigma$(Int(sigma*100))/all_values_$(scenario)_sigma$(Int(sigma*100)).csv"
    #av_file = "results/$(scenario)_sigma$(Int(sigma*100))/all_values_$(scenario)_sigma$(Int(sigma*100)).csv"
    av_all  = isfile(av_file) ? CSV.read(av_file, DataFrame) : nothing
    ...
    run_indices = weight_selection === :all ?
        collect(eachindex(all_weights)) :
        filter(k -> 1 <= k <= length(all_weights), weight_selection)
    ...
    for k in run_indices
        ...
        set_optimizer_attribute(milp, "TimeLimit",      1500)
...
@constraint(milp, [j in J], mu_cm_bud_J[j] <= K_mu_cm_bud_J * r_cm_bud_J[j])
        ...
        @constraint(milp, [t in T], W_t[t] * (VOLL - lambda_e[t]) <= K_stat_ls * r_ls[t])
            ...
            if av_all !== nothing
        avk = av_all[(av_all.run_id .== k) .& (av_all.model .== "milp"), :]
        val_by_name = Dict(string(r.variable) => Float64(r.value) for r in eachrow(avk))


    for v in all_variables(milp)
        is_binary(v) && continue
        haskey(val_by_name, name(v)) && set_start_value(v, val_by_name[name(v)])
    end

    hot = s -> s <= 1e-2 ? 1.0 : 0.0
    sv  = v -> something(start_value(v), 0.0)

    for t in T, i in I
        set_start_value(r_q_I[t,i],    hot(sv(q[t,i])))
        set_start_value(r_g_up_I[t,i], hot(sv(c[i]) - sv(q[t,i])))
    end
    for t in T, j in J
        set_start_value(r_q_J[t,j],    hot(sv(q[t,j])))
        set_start_value(r_g_up_J[t,j], hot(cap[j]*a[(t,j)] - sv(q[t,j])))
    end
    for i in I
        set_start_value(r_c[i],        hot(sv(c[i])))
        set_start_value(r_cap_up[i],   hot(cap[i] - sv(c[i])))
        set_start_value(r_cm_I[i],     hot(sv(c_cm_I[i])))
        set_start_value(r_cm_bud_I[i], hot(alpha[i]*sv(c[i]) - sv(c_cm_I[i])))
    end
    for j in J
        set_start_value(r_cm_J[j],     hot(sv(c_cm_J[j])))
        set_start_value(r_cm_bud_J[j], hot(alpha[j]*cap[j] - sv(c_cm_J[j])))
    end
    for z in Z
        ni = sum((sv(f_trans[p]) for p in pair_keys if p[1]==z); init=0.0) -
             sum((sv(f_trans[p]) for p in pair_keys if p[2]==z); init=0.0)
        set_start_value(r_dem[z],     hot(sv(c_dem[z])))
        set_start_value(r_dem_cap[z], hot(sv(c_dem[z]) - d_c_z[z] + ni))
        set_start_value(r_rho[z],     hot(d_c_z[z] - ni))
    end
    for p in pair_keys
        set_start_value(r_flo[p], hot(sv(f_trans[p]) + mec_pairs[p]))
        set_start_value(r_fhi[p], hot(mec_pairs[p] - sv(f_trans[p])))
    end
    set_start_value(r_sys, hot(sum(sv(c_dem[z]) for z in Z) - d_c))
    for t in T
        set_start_value(r_ls[t], hot(sv(ls[t])))
    end
    else
        @warn "No values found - no warm start" file=av_file run=k
    end
    ...
    # Save results
    # ========================================================================
    run_name = "reruns_$(scenario)_sigma$(Int(sigma * 100))"
    run_dir  = joinpath("results", run_name)
    mkpath(run_dir)
 
    CSV.write(joinpath(run_dir, "rerun_summary_runs_$(scenario)_sigma$(Int(sigma * 100)).csv"),      results)
    CSV.write(joinpath(run_dir, "rerun_prices_$(scenario)_sigma$(Int(sigma * 100)).csv"),            all_prices)
    CSV.write(joinpath(run_dir, "rerun_dispatch_$(scenario)_sigma$(Int(sigma * 100)).csv"),          all_dispatch)
    CSV.write(joinpath(run_dir, "rerun_investment_$(scenario)_sigma$(Int(sigma * 100)).csv"),        all_invest)
    CSV.write(joinpath(run_dir, "rerun_legacy_$(scenario)_sigma$(Int(sigma * 100)).csv"),            all_legacy)
    CSV.write(joinpath(run_dir, "rerun_line_flows_$(scenario)_sigma$(Int(sigma * 100)).csv"),        all_flows)
    CSV.write(joinpath(run_dir, "rerun_redispatch_detail_$(scenario)_sigma$(Int(sigma * 100)).csv"), all_rd_detail)
    CSV.write(joinpath(run_dir, "rerun_curtailment_$(scenario)_sigma$(Int(sigma * 100)).csv"),       all_curt)
    CSV.write(joinpath(run_dir, "rerun_cm_detail_$(scenario)_sigma$(Int(sigma * 100)).csv"),         all_cm)
    CSV.write(joinpath(run_dir, "rerun_cm_diagnostics_$(scenario)_sigma$(Int(sigma * 100)).csv"),    all_cm_diag)
    CSV.write(joinpath(run_dir, "rerun_cm_demand_$(scenario)_sigma$(Int(sigma * 100)).csv"),         all_cm_dem)
    CSV.write(joinpath(run_dir, "rerun_transfers_$(scenario)_sigma$(Int(sigma * 100)).csv"),         all_transfer)
    CSV.write(joinpath(run_dir, "rerun_all_values_$(scenario)_sigma$(Int(sigma * 100)).csv"),         all_values)
