...
for sigma in sigmas
    ...
    # ── Warm-start: Lower-Level-Gleichgewicht als LP (gewichtsunabhängig) ──
    I_in_z_ws = Dict(z => [i for i in I if gen_to_zone[i] == z] for z in Z)
    J_in_z_ws = Dict(z => [j for j in J if gen_to_zone[j] == z] for z in Z)

    ws = Model(Gurobi.Optimizer)
    set_optimizer_attribute(ws, "OutputFlag", 0)

    @variable(ws, ls_ws[t in T] >= 0)
    @variable(ws, q_ws[t in T, g in IJ] >= 0)
    @variable(ws, c_ws[i in I] >= 0)
    @variable(ws, ccmI_ws[i in I] >= 0)
    @variable(ws, ccmJ_ws[j in J] >= 0)
    @variable(ws, cdem_ws[z in Z] >= 0)
    @variable(ws, f_ws[p in pair_keys])

    @expression(ws, ni_ws[z in Z],
        sum(f_ws[p] for p in pair_keys if p[1]==z; init=0.0) -
        sum(f_ws[p] for p in pair_keys if p[2]==z; init=0.0))

    @constraint(ws, [t in T], ls_ws[t] <= D_max[t])
    @constraint(ws, [t in T], sum(q_ws[t,g] for g in IJ) + ls_ws[t] == D_max[t])
    @constraint(ws, [t in T, i in I], q_ws[t,i] <= c_ws[i])
    @constraint(ws, [t in T, j in J], q_ws[t,j] <= cap[j]*a[(t,j)])
    @constraint(ws, [i in I], c_ws[i] <= cap[i])
    @constraint(ws, [i in I], ccmI_ws[i] <= alpha[i]*c_ws[i])
    @constraint(ws, [j in J], ccmJ_ws[j] <= alpha[j]*cap[j])
    @constraint(ws, [p in pair_keys], f_ws[p] + mec_pairs[p] >= 0)
    @constraint(ws, [p in pair_keys], mec_pairs[p] - f_ws[p] >= 0)
    @constraint(ws, [z in Z], d_c_z[z] - ni_ws[z] >= 0)
    @constraint(ws, [z in Z], cdem_ws[z] - d_c_z[z] + ni_ws[z] >= 0)
    @constraint(ws, [z in Z],
        sum(ccmI_ws[i] for i in I_in_z_ws[z]; init=0.0) +
        sum(ccmJ_ws[j] for j in J_in_z_ws[z]; init=0.0) - cdem_ws[z] == 0)
    @constraint(ws, sum(cdem_ws[z] for z in Z) >= d_c)

    @objective(ws, Min,
        sum(W_t[t]*vc[i]*q_ws[t,i] for t in T for i in I) +
        sum(W_t[t]*vc[j]*q_ws[t,j] for t in T for j in J) +
        sum(fc[i]*c_ws[i] for i in I) +
        sum(W_t[t]*VOLL*ls_ws[t] for t in T))

    optimize!(ws)
    @assert has_values(ws) "Warm-start LP infeasible"

    qv    = Dict((t,g) => value(q_ws[t,g]) for t in T, g in IJ)
    cv0   = Dict(i => value(c_ws[i])    for i in I)
    lsv   = Dict(t => value(ls_ws[t])   for t in T)
    ccmIv = Dict(i => value(ccmI_ws[i]) for i in I)
    ccmJv = Dict(j => value(ccmJ_ws[j]) for j in J)
    cdemv = Dict(z => value(cdem_ws[z]) for z in Z)
    fv    = Dict(p => value(f_ws[p])    for p in pair_keys)
    niv   = Dict(z => value(ni_ws[z])   for z in Z)
    ...
    for (k, w) in enumerate(all_weights)
        ...
        set_optimizer_attribute(milp, "TimeLimit",      120)
        ...
        set_optimizer_attribute(milp, "MIPFocus",     1)  
        set_optimizer_attribute(milp, "NumericFocus", 2)
...
# 0 <= q[t,i] ⊥ stat_q_I >= 0    (q ≤ c ≤ cap[i])
@constraint(milp, [t in T, i in I], q[t, i] <= cap[i] * (1 - r_q_I[t, i]))
@constraint(milp, [t in T, i in I],
    -W_t[t]*(lambda_e[t]-vc[i]) + mu_g_up_I[t,i] <= K_stat_q_I * r_q_I[t,i])

# 0 <= (q_max_I - q) ⊥ mu_g_up_I >= 0
@constraint(milp, [t in T, i in I], q_max_I[t,i] - q[t,i] <= cap[i] * (1 - r_g_up_I[t,i]))
@constraint(milp, [t in T, i in I], mu_g_up_I[t,i] <= K_mu_g_I * r_g_up_I[t,i])

# 0 <= c ⊥ stat_c >= 0
@constraint(milp, [i in I], c[i] <= cap[i] * (1 - r_c[i]))
@constraint(milp, [i in I],
    fc[i] - sum(mu_g_up_I[t,i] for t in T) + mu_cap_up[i] - mu_cm_bud_I[i]*alpha[i]
    <= K_stat_c * r_c[i])

# 0 <= (cap - c) ⊥ mu_cap_up >= 0
@constraint(milp, [i in I], cap[i] - c[i] <= cap[i] * (1 - r_cap_up[i]))
@constraint(milp, [i in I], mu_cap_up[i] <= K_mu_cap * r_cap_up[i])

# 0 <= c_cm_I ⊥ stat_cm_I >= 0
@constraint(milp, [i in I], c_cm_I[i] <= alpha[i]*cap[i] * (1 - r_cm_I[i]))
@constraint(milp, [i in I],
    -lambda_c[gen_to_zone[i]] + mu_cm_bud_I[i] <= K_stat_cm * r_cm_I[i])

# 0 <= (budget slack I) ⊥ mu_cm_bud_I >= 0
@constraint(milp, [i in I],
    alpha[i]*c[i] - c_cm_I[i] <= alpha[i]*cap[i] * (1 - r_cm_bud_I[i]))
@constraint(milp, [i in I], mu_cm_bud_I[i] <= K_mu_cm_bud_I * r_cm_bud_I[i])

# 0 <= q[t,j] ⊥ stat_q_J >= 0    (q ≤ cap[j]*a)
@constraint(milp, [t in T, j in J], q[t,j] <= cap[j]*a[(t,j)] * (1 - r_q_J[t,j]))
@constraint(milp, [t in T, j in J],
    -W_t[t]*(lambda_e[t]-vc[j]) + mu_g_up_J[t,j] <= K_stat_q_J * r_q_J[t,j])

# 0 <= (q_max_J - q) ⊥ mu_g_up_J >= 0
@constraint(milp, [t in T, j in J],
    q_max_J[t,j] - q[t,j] <= cap[j]*a[(t,j)] * (1 - r_g_up_J[t,j]))
@constraint(milp, [t in T, j in J], mu_g_up_J[t,j] <= K_mu_g_J * r_g_up_J[t,j])

# 0 <= c_cm_J ⊥ stat_cm_J >= 0
@constraint(milp, [j in J], c_cm_J[j] <= alpha[j]*cap[j] * (1 - r_cm_J[j]))
@constraint(milp, [j in J],
    -lambda_c[gen_to_zone[j]] + mu_cm_bud_J[j] <= K_stat_cm * r_cm_J[j])

# 0 <= (budget slack J) ⊥ mu_cm_bud_J >= 0
@constraint(milp, [j in J],
    alpha[j]*cap[j] - c_cm_J[j] <= alpha[j]*cap[j] * (1 - r_cm_bud_J[j]))
@constraint(milp, [j in J], mu_cm_bud_J[j] <= K_mu_cm_bud_J * r_cm_bud_J[j])
        ...
        # ── Warm start aus dem Gleichgewichts-LP ──
        tol = 1e-3
        hot(s) = s <= tol ? 1.0 : 0.0

        for t in T, i in I
            set_start_value(r_q_I[t,i],    hot(qv[(t,i)]))
            set_start_value(r_g_up_I[t,i], hot(cv0[i] - qv[(t,i)]))
        end
        for t in T, j in J
            set_start_value(r_q_J[t,j],    hot(qv[(t,j)]))
            set_start_value(r_g_up_J[t,j], hot(cap[j]*a[(t,j)] - qv[(t,j)]))
        end
        for i in I
            set_start_value(r_c[i],        hot(cv0[i]))
            set_start_value(r_cap_up[i],   hot(cap[i] - cv0[i]))
            set_start_value(r_cm_I[i],     hot(ccmIv[i]))
            set_start_value(r_cm_bud_I[i], hot(alpha[i]*cv0[i] - ccmIv[i]))
        end
        for j in J
            set_start_value(r_cm_J[j],     hot(ccmJv[j]))
            set_start_value(r_cm_bud_J[j], hot(alpha[j]*cap[j] - ccmJv[j]))
        end
        for z in Z
            set_start_value(r_dem[z],     hot(cdemv[z]))
            set_start_value(r_dem_cap[z], hot(cdemv[z] - d_c_z[z] + niv[z]))
            set_start_value(r_rho[z],     hot(d_c_z[z] - niv[z]))
        end
        for p in pair_keys
            set_start_value(r_flo[p], hot(fv[p] + mec_pairs[p]))
            set_start_value(r_fhi[p], hot(mec_pairs[p] - fv[p]))
        end
        set_start_value(r_sys, hot(sum(cdemv[z] for z in Z) - d_c))
        for t in T
            set_start_value(r_ls[t], hot(lsv[t]))
        end

        for t in T
            set_start_value(ls[t], lsv[t])
            for g in IJ; set_start_value(q[t,g], qv[(t,g)]); end
        end
        for i in I; set_start_value(c[i], cv0[i]); set_start_value(c_cm_I[i], ccmIv[i]); end
        for j in J; set_start_value(c_cm_J[j], ccmJv[j]); end
        for z in Z; set_start_value(c_dem[z], cdemv[z]); end
        for p in pair_keys; set_start_value(f_trans[p], fv[p]); end
        ...
        # ── Extract results ──────────────────────────────────────────────────
        if !has_values(milp)
            st = termination_status(milp)
            println("MPEC FAILED: ", st,
            " | result_count=", result_count(milp),
            " | bound=", try objective_bound(milp) catch; NaN end)
            if st == MOI.INFEASIBLE || st == MOI.INFEASIBLE_OR_UNBOUNDED
                compute_conflict!(milp)               # Gurobi IIS
                for c in all_constraints(milp; include_variable_in_set_constraints=false)
                    MOI.get(milp, MOI.ConstraintConflictStatus(), c) == MOI.IN_CONFLICT && println("  IIS: ", c)
                end
            end
            push!(results, vcat(
