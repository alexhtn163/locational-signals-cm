...
function load_data(scenario="nl20"; data_dir="data", overrides...)
    ...
    base = (
        invest_gen_df = CSV.read(p(:invest_generators), DataFrame, ; stringtype=String),
        legacy_gen_df = CSV.read(p(:legacy_generators), DataFrame, ; stringtype=String),
        renewable_df  = CSV.read(p(:renewable_gens), DataFrame, ; stringtype=String),
        dem_tot_df    = CSV.read(p(:total_load),     DataFrame, ; stringtype=String),
        dem_df        = CSV.read(p(:load_per_bus),   DataFrame, ; stringtype=String),
        ptdf_df       = CSV.read(p(:ptdf),           DataFrame, ; stringtype=String),
        bus_order_df  = CSV.read(p(:bus_order),      DataFrame, ; stringtype=String),
        line_order_df = CSV.read(p(:line_order),     DataFrame, ; stringtype=String),
        fixed_om_df   = CSV.read(p(:fixed_om),       DataFrame, ; stringtype=String),
        cf_df         = CSV.read(p(:renewable_cf),   DataFrame, ; stringtype=String),
    )
    ...
    if haskey(cfg, :rep_profiles)
        extras = merge(extras, (
            rep_profiles_df  = CSV.read(p(:rep_profiles),  DataFrame, ; stringtype=String),
            rep_decisions_df = CSV.read(p(:rep_decisions), DataFrame, ; stringtype=String),
        ))
    end
 
    if haskey(cfg, :mec_profiles)
        extras = merge(extras, (
            mec_df = CSV.read(p(:mec_profiles), DataFrame, ; stringtype=String),
        ))
    end
...
function build_generators(data, rep, bus_to_idx;
    ...
    # Base fixed cost per technology
    fc_by_tech = Dict(
        row.tech => Float64(row.fixed_om_eur_MW_hour) * total_hours
        for row in eachrow(fixed_om_df)
    )

    # Default missing fixed-cost entries to zero
    for t in unique(values(tech))
        haskey(fc_by_tech, t) || (fc_by_tech[t] = 0.0)
    end

    # Generator-specific fixed costs
    fc = Dict{String,Float64}()

    # Investable generators: apply markup if present
    for row in eachrow(invest_gen_df)
        g = string(row.name)
        base_fc = fc_by_tech[row.carrier]

        markup = if "fc-markup" in names(invest_gen_df)
            Float64(row[Symbol("fc-markup")])
        elseif "fc_markup" in names(invest_gen_df)
            Float64(row.fc_markup)
        else
            1.0
        end
    ...
    # Legacy generators: no markup
    for row in eachrow(legacy_gen_df)
        g = string(row.name)
        fc[g] = fc_by_tech[row.carrier]
    end
...
function build_zones(data, net, gen_data; sigma::Float64 = 1.0)
    ...
    if hasproperty(data, :mec_df)
        ...
        for ((z1, z2), val) in MEC
            z1 < z2 || continue          # keep each unordered pair once
            MEC_pairs[(z1, z2)] = val    # val already = sigma * TC
        end
        #=
